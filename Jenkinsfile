      ...
      steps {
        container('helm'){
          script {
            ...
            sh "helm template -f helm-values/values-bookinfo-${ENV_NAME}-productpage.yaml \
            --set extraEnv.COMMIT_ID=${scmVars.GIT_COMMIT} \
            --namespace ${ENV_NAME} bookinfo-${ENV_NAME}-productpage helm/ \
      ...
      steps {
        container('java-node'){
          script {
            ...
            // sh ''' cd /src && ruby install --package-lock && cd ../'''
        ...
        container('docker'){
          script {
            docker.withRegistry('https://ghcr.io', 'registry-bookinfo'){ //registry-bookinfo is user with token
                          // build and push
              docker.build('ghcr.io/mercurial963/bookinfo-productpage:${ENV_NAME}').push()
      ...
      steps {
        container('jnlp'){
          script {
            ...
            // def image = 'ghcr.io/mercurial963/bookinfo-productpage:${ENV_NAME}'
                         
            writeFile file: 'anchore_images', text: "ghcr.io/mercurial963/bookinfo-productpage:${ENV_NAME}"
        ...
        container('helm'){
          script {
            // withKubeConfig([credentialsId: 'config']){ //add kubeconfig to secret file
              sh "helm upgrade --install -f helm-values/values-bookinfo-${ENV_NAME}-productpage.yaml --wait \
              --set extraEnv.COMMIT_ID=${scmVars.GIT_COMMIT} \
              --namespace ${ENV_NAME} bookinfo-${ENV_NAME}-productpage helm/"
              // }// withCredentials
